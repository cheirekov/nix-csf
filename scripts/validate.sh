...
echo "[nix-csf] running x86_64 lightweight checks (version + eval + profile eval + netdata eval + nat eval + forwarding eval + egress eval + control-plane eval + lfd detector eval + lfd detector-pack eval + fail2ban adapter eval + monitoring eval + csf import check + shellcheck + control-plane lint + monitoring pack)"
nix build \
