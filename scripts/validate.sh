...
echo "[nix-csf] running x86_64 lightweight checks (version + eval + profile eval + netdata eval + nat eval + forwarding eval + egress eval + control-plane eval + lfd detector eval + lfd detector-pack eval + fail2ban adapter eval + monitoring eval + csf import check + policy compile check + shellcheck + control-plane lint + monitoring pack)"
nix build \
  ...
  "path:${ROOT_DIR}#checks.x86_64-linux.eval-nat" \
  "path:${ROOT_DIR}#checks.x86_64-linux.eval-forwarding" \
  "path:${ROOT_DIR}#checks.x86_64-linux.eval-egress" \
  "path:${ROOT_DIR}#checks.x86_64-linux.eval-control-plane" \
  "path:${ROOT_DIR}#checks.x86_64-linux.eval-lfd-detector" \
  "path:${ROOT_DIR}#checks.x86_64-linux.eval-lfd-detector-pack" \
  ...
  "path:${ROOT_DIR}#checks.x86_64-linux.policy-compile-check" \
